...
class MCPClient:
    def __init__(self, api_key: Optional[str] = None):
        """
        Initializes the MCP client and configure Gemini API.
        """
    ...
    async def connect_to_server(self, server_command: str):
        """
        Connects to the MCP server using the provided command.
        """
        # This allows us to execute the correct command to start the MCP server
        command = "python" if server_command.endswith(".py") else "node"
        ...
        stdio_transport = await self.exit_stack.enter_async_context(stdio_client(server_parameter))
